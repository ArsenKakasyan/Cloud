...
class FileController{
    ...
    async uploadFile(req, res) {
        try {
            ...
            if(user.usedSpace + file.size > user.diskSpace){
                return res.status(400).json({message: "There's no space on this disk"})
            }
    ...
    async downloadFile(req, res) {
        try {
            ...
            return res.status(400).json({message: 'Download error: file doesnt exist'})
        }catch (e) {
            console.log(e)
            res.status(500).json({message: "Download error: something went wrong"})
        }
    }
